...
if(isset($_POST['update_product'])){
   ...
   mysqli_query($conn, "UPDATE `cproducts` SET name = '$name', details = '$details', price = '$price' WHERE id = '$update_p_id'") or die('query failed');
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         mysqli_query($conn, "UPDATE `cproducts` SET image = '$image' WHERE id = '$update_p_id'") or die('query failed');
...
<head>
   ...
   <title>Admin|Update Customized Product</title>
...
<?php
   ...
   $select_products = mysqli_query($conn, "SELECT * FROM `cproducts` WHERE id = '$update_id'") or die('query failed');
...
<form action="" method="post" enctype="multipart/form-data">
   ...
   <a href="admin_customized.php" class="option-btn">go back</a>
</form>
